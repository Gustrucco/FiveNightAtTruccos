...
namespace AlumnoEjemplos.NeneMalloc.Utils.GrillaRegular
{
    /// <summary>
    ...
    public class GrillaRegular
    {
        ...
        float CELL_WIDTH = 90;
        float CELL_HEIGHT = 125;
        float CELL_LENGTH = 100;


        List<TgcMesh> modelos;
        TgcBoundingBox sceneBounds;
        public List<TgcDebugBox> debugBoxes;
        ...
        /// </summary>
        public void findVisibleMeshes(TgcFrustum frustum)
        {
            for (int x = 0; x < grid.GetUpperBound(0); x++)
            {
                for (int y = 0; y < grid.GetUpperBound(1); y++)
